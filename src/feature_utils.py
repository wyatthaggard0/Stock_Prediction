...
def extract_features():
    ...
    stk_tickers = ['RIVN', 'TSLA', 'LCID', 'NIO']
    ...
    Y = np.log(stk_data.loc[:, ('Adj Close', 'RIVN')]).diff(return_period).shift(-return_period)
    Y.name = Y.name[-1]+'_Future'
    
    X1 = np.log(stk_data.loc[:, ('Adj Close', ('TSLA', 'LCID', 'NIO'))]).diff(return_period)
